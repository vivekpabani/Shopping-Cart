    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.stock != null && item.name != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.stock, item.name, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (stock, name, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(stock, name, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.getTotalPrice = function (stock) {
    var total = 0;
...
// get the total price for all items currently in the cart
shoppingCart.prototype.getTotalCount = function (stock) {
    var count = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (stock == null || item.stock == stock) {
...
shoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        var ctr = i + 1;
        data["item_number_" + ctr] = item.stock;
...
shoppingCart.prototype.checkoutGoogle = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        var ctr = i + 1;
        data["item_name_" + ctr] = item.stock;
...
// items in the cart
//
function cartItem(stock, name, price, quantity) {
    this.stock = stock;
    this.name = name;
    this.price = price * 1;
